...
// Download file
				if(s3Client.doesObjectExist(bucketName, fileObjKeyName))
				{
					try
					{
						...
						ctx.status(200);
						Metrics.counter("s3storagefrontend.get", "resourcename", "/delete", "remoteAddr", ctx.req.getRemoteAddr(), "remoteHost", ctx.req.getRemoteHost(), "localAddr", ctx.req.getLocalAddr(), "localName", ctx.req.getLocalName()).increment();
					}
					catch(com.amazonaws.services.s3.model.AmazonS3Exception axp)
					{
						ctx.status(axp.getStatusCode());
						ctx.result(axp.getMessage());
						if(CLASS_LOGGER.isEnabledFor(Priority.ERROR))CLASS_LOGGER.error(axp.getMessage());
						Metrics.counter("s3storagefrontend.get", "resourcename","/delete","httpstatus",java.lang.Integer.toString(axp.getStatusCode()),"error",axp.getMessage(),"remoteAddr",ctx.req.getRemoteAddr(),"remoteHost",ctx.req.getRemoteHost(),"localAddr",ctx.req.getLocalAddr(),"localName",ctx.req.getLocalName()).increment();
					}
				}
				else
				{
					...
					ctx.result("The specified key does not exist ("+fileObjKeyName+")!");
					if(CLASS_LOGGER.isEnabledFor(Priority.ERROR))CLASS_LOGGER.error("The specified key does not exist ("+fileObjKeyName+")!");
					Metrics.counter("s3storagefrontend.get", "resourcename","/delete","httpstatus","404","error","object key does not exist","remoteAddr",ctx.req.getRemoteAddr(),"remoteHost",ctx.req.getRemoteHost(),"localAddr",ctx.req.getLocalAddr(),"localName",ctx.req.getLocalName()).increment();
				}
			}
			else
			{
				...
				ctx.result("request is not multipart/form-data ");
				if(CLASS_LOGGER.isEnabledFor(Priority.ERROR))CLASS_LOGGER.error("request is not multipart/form-data ");
				Metrics.counter("s3storagefrontend.get", "resourcename","/delete","httpstatus","500","error","uuid not set","remoteAddr",ctx.req.getRemoteAddr(),"remoteHost",ctx.req.getRemoteHost(),"localAddr",ctx.req.getLocalAddr(),"localName",ctx.req.getLocalName()).increment();
			}
		});
